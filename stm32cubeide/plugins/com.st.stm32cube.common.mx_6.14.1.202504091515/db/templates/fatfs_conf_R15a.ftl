...
/**
  ******************************************************************************
  *  FatFs - Generic FAT file system module  R0.15a (C)ChaN, 2024
  ******************************************************************************
...
[/#if]
[#if cmsisrtosInUse == 1]
#include "cmsis_os2.h"    /* _FS_REENTRANT set to 1 and CMSIS API chosen */
[/#if]
...
[#-- Global variables following the order of appearance in 0.15a --]
[#if SWIP.defines??]
    [#list SWIP.defines as definition]
...
#define hUSB_Host ${definition.value}
        [/#if]
      [/#if] 
      [#-- Parameters for Function Configurations --]
      ...
      [/#if]
      [#if definition.name=="_PRINT_LLI"]
          [#assign valuePrintLLI = definition.value]
      [/#if]
      [#if definition.name=="_PRINT_FLOAT"]
          [#assign valuePrintFLOAT = definition.value]
      [/#if]
      ...
      [/#if]      
      [#if definition.name=="_LBA64"]
          [#assign valueLBA64 = definition.value]
      [/#if]
      [#if definition.name=="_MIN_GPT"]
          [#assign valueMinGPT = definition.value]
      [/#if]
...
#define FF_USE_STRFUNC	${valueUseStrfunc}
#define FF_PRINT_LLI	${valuePrintLLI}
#define FF_PRINT_FLOAT	${valuePrintFLOAT}
...
/  1024, 2048 or 4096) Always set both 512 for most systems, generic memory cards and
/  harddisks, but a larger value may be required for on-board flash memory and some
...
/  In tiny configuration, size of file object (FIL) is shrunk to FF_MAX_SS bytes.
